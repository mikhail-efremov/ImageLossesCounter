...
namespace MeshCollision
{
  public class KMeansDemo
  {
    public static void Start(List<Line> lines)
    {
      ...
      {
        var array = line.Points.ToArray();
        map.AddRange(array);
      }

      var ints = new Double[map.Count][];
      for (var i = 0; i < map.Count; i++)
      {
        ints[i] = new[] {(double)map[i].X, map[i].Y};
      }

      Start(ints);
    }

    public static ClasteringData Start(List<Point> points)
    {
    ...
    public static ClasteringData Start(double[][] points)
    {

      Console.WriteLine("Raw unclustered data:\n");
      Console.WriteLine("    Height Weight");
      Console.WriteLine("-------------------");
      ShowData(points, 1, true, true);

      int numClusters = 5;
      Console.WriteLine("\nSetting numClusters to " + numClusters);

      var clustering = Cluster(points, numClusters); // this is it
    ...
    static ClasteringData ShowClustered(double[][] data, int[] clustering, int numClusters, int decimals)
    {
      return new ClasteringData
      {
        Data = data,
